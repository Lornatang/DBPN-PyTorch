...
def build_model(upscale_factor: int) -> nn.Module:
    model = DBPN(upscale_factor=upscale_factor).to(config.device)
...
def resume_checkpoint(model) -> None:
    if config.resume:
        if config.resume_weight != "":
            ...
            pretrained_state_dict = torch.load(config.resume_weight, map_location=lambda storage, loc: storage)
...
def train(model, train_dataloader, psnr_criterion, pixel_criterion, optimizer, epoch, scaler, writer) -> None:
    ...
    for index, (lr, bic, hr) in enumerate(train_dataloader):
        # measure data loading time
        data_time.update(time.time() - end)

        lr = lr.to(config.device, non_blocking=True)
        bic = bic.to(config.device, non_blocking=True)
...
def validate(model, valid_dataloader, psnr_criterion, epoch, writer) -> float:
    ...
    with torch.no_grad():
        for index, (lr, _, hr) in enumerate(valid_dataloader):
            lr = lr.to(config.device, non_blocking=True)
            hr = hr.to(config.device, non_blocking=True)
            ...
            # measure accuracy and record loss
            avg_psnr += 10. * torch.log10(1. / psnr_criterion(sr, hr))

    # Print evaluation indicators.
    avg_psnr /= len(valid_dataloader)
    writer.add_scalar("Valid/PSNR", avg_psnr, epoch + 1)
    print(f"* Avg. PSNR: {avg_psnr:4.2f} dB.\n")
